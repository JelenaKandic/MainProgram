...
from roulette_service import show_roulette_options
...
def show_step2(root, username):
    ...
    tk.Button(root, text="Find Restaurants", command=lambda: find_restaurants_and_show_result(
        mood_entry, distance_entry, price_entry, rating_entry, result_text)).pack(pady=(0, 10))
    ...
    tk.Button(frame2, text="Go", width=6, command=lambda: show_roulette_options(root)).pack(side="left", padx=(0, 10))
